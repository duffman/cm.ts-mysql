...
import { DBLogger }                  from '@putteDb/db-logger';
...
export class DBKernel implements IDBKernel {
	...
	//private settings: IDbSettings;
	private connLost: boolean = false;

	constructor() {}

	/*
	public initDbKernel(dbSettings: IDbSettings) {
		console.log('INIT DB KERNEL');
	...
	*/
	...
	public createConnection(openConnection: boolean = true): Connection {
		if (!AppSettings.dbSettings) {
			DBLogger.logErrorMessage('DBKernel :: createConnection :: kernel not initialized - settings missing');
			return;
		}
		...
		try {
			conn = mysql.createConnection({
				host     : AppSettings.dbSettings.dbHost,
				user     : AppSettings.dbSettings.dbUser,
				password : AppSettings.dbSettings.dbPass,
				database : AppSettings.dbSettings.dbName
			});
